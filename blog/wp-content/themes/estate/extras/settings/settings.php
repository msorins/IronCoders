...
function siteorigin_settings_admin_menu() {
	$theme = wp_get_theme();
	$page = add_theme_page(
		sprintf(__( '%s Theme Settings', 'estate' ), $theme->get('Name') ),
		sprintf(__( 'Theme Settings', 'estate' ), $theme->get('Name') ),
		'edit_theme_options',
		'theme_settings_page',
...
function siteorigin_settings_render() {
	if( version_compare( get_bloginfo('version'), '3.4', '<' ) ) {
		?><div class="wrap"><div id="setting-error-settings_updated" class="updated settings-error"><p><strong><?php _e('Please update to the latest version of WordPress to use theme settings.', 'estate') ?></strong></p></div></div><?php
		return;
	}
...
function siteorigin_settings_field( $args ) {
	...
	switch ( $args['type'] ) {
		case 'checkbox' :
			...
			<label for="<?php echo esc_attr( $field_id ) ?>"><?php echo esc_attr( !empty( $args['label'] ) ? $args['label'] : __( 'Enabled', 'estate' ) ) ?></label>
			<?php
			break;
		case 'text' :
		case 'number' :
		...
		case 'media':
			if(version_compare(get_bloginfo('version'), '3.5', '<')){
				printf(__('You need to <a href="%s">upgrade</a> to WordPress 3.5 to use media fields', 'estate'), admin_url('update-core.php'));
				break;
			}
			...
			$choose_title = empty($args['choose']) ? __('Choose Media', 'estate') : $args['choose'];
			$update_button = empty($args['update']) ? __('Set Media', 'estate') : $args['update'];
			...
			?>
				<div class="media-field-wrapper">
					...
					<a href="#" class="media-remove-button"><?php _e('Remove', 'estate') ?></a>
				</div>
		...
		case 'gallery' :
			...
			<input
				...
				type="text"
				value="<?php echo esc_attr( $current ) ?>" />
			<a href="#" class="so-settings-gallery-edit"><?php _e('Select Images', 'estate') ?></a>
			<?php
			break;
		...
		case 'pages' :
			...
			?>
			<select id="<?php echo esc_attr( $field_id ) ?>" name="<?php echo esc_attr( $field_name ) ?>">
				<option value="0"><?php _e('None', 'estate') ?></option>
		...
		case 'widget' :
			...
			if( !class_exists($args['widget_class']) ) {
				...
				printf( __('This field requires the %s plugin. ', 'estate'), $args['plugin_name']);
				if( function_exists('siteorigin_plugin_activation_install_url') ) {
					...
					printf( __('<a href="%s" target="_blank">Install %s</a> now. ', 'estate'), $install_url, $args['plugin_name']);
				}
				?></div>
				...
				echo '<div class="so-settings-widget-form"><a href="#" class="so-settings-widget-edit" data-is-setup="0" data-form="'.esc_attr($form).'">' . __('Edit', 'estate') . '</a></div>';
				?><input type="hidden" id="<?php echo esc_attr( $field_id ) ?>" name="<?php echo esc_attr( $field_name ) ?>" value="<?php echo esc_attr( serialize( $current ) ) ?>" /><?php
			}
			break;

		default :
			_e( 'Unknown Field Type', 'estate' );
			break;
	}
...
function siteorigin_settings_change_message() {
	if ( get_theme_mod( 'siteorigin_settings_changed' ) ) {
		...
		<div id="setting-updated" class="updated">
			<p>
				<strong><?php _e( 'Settings saved.', 'estate' ) ?></strong>
			</p>
		</div>
		<?php
...
function siteorigin_settings_theme_help(){
	...
	$text = sprintf(
		__( "Read %s's <a href='%s'>theme documentation</a> for help with these settings.", 'estate' ),
		ucfirst($theme_name),
...
function siteorigin_settings_media_view_strings($strings, $post){
	...
	$strings['insertIntoPost'] = __('Set Media File', 'estate');
