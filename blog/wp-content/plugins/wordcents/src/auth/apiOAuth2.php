...
class apiOAuth2 extends apiAuth {
  ...
  public function authenticate($service) {
  	global $apiCredentials,$WordCents_Version;
    ...
    if (isset($apiCredentials['RedirectUri']) && substr($apiCredentials['RedirectUri'].'nullX0noneNothing', 0, 17) == 'http://wordpress.')
    	header('Location: '.$apiCredentials['RedirectUri'].'?'.$authUrl.'&p=WordCents&v='.$WordCents_Version.'&d='.urlencode('http://' . $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']));
    else
    	die('</form><form action="" method="get" name="WordCents_code_form"><a target="_blank" href="'.$authUrl.'">Get your Auth Token Code here</a> and enter it below:<br /><input type="text" name="code" value="" style="width: 100%;" /><br /><input type="hidden" name="auth_plugin" value="wordcents" /><input type="submit" class="button-primary" value="Save Token Code" /></form>');
  } 
