...
<?php if ($this->_rootref['SEARCHED_QUERY']) {  ?> <p class="subtitle"><?php echo ((isset($this->_rootref['L_SEARCHED_QUERY'])) ? $this->_rootref['L_SEARCHED_QUERY'] : ((isset($user->lang['SEARCHED_QUERY'])) ? $user->lang['SEARCHED_QUERY'] : '{ SEARCHED_QUERY }')); ?>: <strong><?php echo (isset($this->_rootref['SEARCHED_QUERY'])) ? $this->_rootref['SEARCHED_QUERY'] : ''; ?></strong></p><?php } if ($this->_rootref['IGNORED_WORDS']) {  ?> <p class="subtitle"><?php echo ((isset($this->_rootref['L_IGNORED_TERMS'])) ? $this->_rootref['L_IGNORED_TERMS'] : ((isset($user->lang['IGNORED_TERMS'])) ? $user->lang['IGNORED_TERMS'] : '{ IGNORED_TERMS }')); ?>: <strong><?php echo (isset($this->_rootref['IGNORED_WORDS'])) ? $this->_rootref['IGNORED_WORDS'] : ''; ?></strong></p><?php } if ($this->_rootref['SEARCH_TOPIC']) {  ?>

	<p><a class="left-box <?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>" href="<?php echo (isset($this->_rootref['U_SEARCH_TOPIC'])) ? $this->_rootref['U_SEARCH_TOPIC'] : ''; ?>"><?php echo ((isset($this->_rootref['L_RETURN_TO'])) ? $this->_rootref['L_RETURN_TO'] : ((isset($user->lang['RETURN_TO'])) ? $user->lang['RETURN_TO'] : '{ RETURN_TO }')); ?>: <?php echo (isset($this->_rootref['SEARCH_TOPIC'])) ? $this->_rootref['SEARCH_TOPIC'] : ''; ?></a></p>
<?php } else { ?>
...
<?php } if ($this->_rootref['PAGINATION'] || $this->_rootref['SEARCH_MATCHES'] || $this->_rootref['PAGE_NUMBER']) {  ?>
	...
	<?php if ($this->_rootref['SEARCH_MATCHES']) {  if (! $this->_rootref['SEARCH_IN_RESULTS']) {  ?>

			<?php echo ((isset($this->_rootref['L_SEARCH_IN_RESULTS'])) ? $this->_rootref['L_SEARCH_IN_RESULTS'] : ((isset($user->lang['SEARCH_IN_RESULTS'])) ? $user->lang['SEARCH_IN_RESULTS'] : '{ SEARCH_IN_RESULTS }')); ?>

		<?php } if ($this->_rootref['SEARCH_IN_RESULTS']) {  ?>

		<div class="search">
				<input placeholder="<?php echo ((isset($this->_rootref['L_SEARCH_IN_RESULTS'])) ? $this->_rootref['L_SEARCH_IN_RESULTS'] : ((isset($user->lang['SEARCH_IN_RESULTS'])) ? $user->lang['SEARCH_IN_RESULTS'] : '{ SEARCH_IN_RESULTS }')); ?>" type="text" name="add_keywords" value="" class="keywords" />
		...
		<div class="pagination">
			<span class="matches"><?php echo (isset($this->_rootref['SEARCH_MATCHES'])) ? $this->_rootref['SEARCH_MATCHES'] : ''; ?></span>
...
<?php } if ($this->_rootref['S_SHOW_TOPICS']) {  if (sizeof($this->_tpldata['searchresults'])) {  ?>
		...
		<?php $_searchresults_count = (isset($this->_tpldata['searchresults'])) ? sizeof($this->_tpldata['searchresults']) : 0;if ($_searchresults_count) {for ($_searchresults_i = 0; $_searchresults_i < $_searchresults_count; ++$_searchresults_i){$_searchresults_val = &$this->_tpldata['searchresults'][$_searchresults_i]; ?>
			...
			<li class="row<?php if (!($_searchresults_val['S_ROW_COUNT'] & 1)  ) {  ?> bg1<?php } else { ?> bg2<?php } ?>">
				<dl class="icon" style="background-image: url(<?php echo $_searchresults_val['TOPIC_FOLDER_IMG_SRC']; ?>); background-repeat: no-repeat;">
					...
					<dd class="posts-views"><?php echo $_searchresults_val['TOPIC_REPLIES']; ?> / <?php echo $_searchresults_val['TOPIC_VIEWS']; ?></dd>
					<dd class="lastpost"><span>
						...
						<?php if (! $this->_rootref['S_IS_BOT']) {  ?><a href="<?php echo $_searchresults_val['U_LAST_POST']; ?>"><i class="fa  fa-chevron-right"></i></a> <?php } ?><br /><?php echo $_searchresults_val['LAST_POST_TIME']; ?><br /> </span>
					</dd>
				</dl>
			</li>
		<?php }} ?>
	...
	<?php } } else { $_searchresults_count = (isset($this->_tpldata['searchresults'])) ? sizeof($this->_tpldata['searchresults']) : 0;if ($_searchresults_count) {for ($_searchresults_i = 0; $_searchresults_i < $_searchresults_count; ++$_searchresults_i){$_searchresults_val = &$this->_tpldata['searchresults'][$_searchresults_i]; ?>
		...
		<div class="post <?php if (($_searchresults_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } if ($_searchresults_val['S_POST_REPORTED']) {  ?> reported<?php } ?>">
	...
	<?php } if (! $_searchresults_val['S_IGNORE_POST']) {  ?>
		...
		<div class="jump2post"><a href="<?php echo $_searchresults_val['U_VIEW_POST']; ?>"><?php echo ((isset($this->_rootref['L_JUMP_TO_POST'])) ? $this->_rootref['L_JUMP_TO_POST'] : ((isset($user->lang['JUMP_TO_POST'])) ? $user->lang['JUMP_TO_POST'] : '{ JUMP_TO_POST }')); ?></a></div>
	<?php } ?>
